...
const outFile = './create/props.sql'
...
function append (what) {
  fs.appendFileSync(path.resolve(basePath, outFile), what)
}
...
/* GO */
show('Start..', true)
mkdirp.sync(path.resolve(basePath, path.parse(outFile).dir))
fs.writeFileSync(path.resolve(basePath, outFile), '')
...
appendFile('schemas.sql', function (data) {
  ...
  data = data.replace(/\n\tand exists\(select p.principal_id from sys.database_principals p where p.name = '(.+?)'\) begin/g, 'begin') // eslint-disable-line no-control-regex
